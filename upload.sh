	...
	if [ ! -z "$FILE" ]; then
		if [ -f "$FILE" ];then
			uploadFile "$FILE" "$FOLDER_ID" "$ACCESS_TOKEN"
				...
				FOLDER_ID=`createDirectory "$FOLDERNAME" "$FOLDER_ID" "$ACCESS_TOKEN"`
				for file in $(find "$FILE" -maxdepth 1 -mindepth 1);
				do
					traverseFileUpload "$file" "$FOLDER_ID" "$ACCESS_TOKEN"
				done
		fi
	fi
}

traverseFileUpload "$FILE" "$ROOT_FOLDER" "$ACCESS_TOKEN"
